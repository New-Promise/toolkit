...
(function(window, document, jQuery, CodeMirror, $) {
    ...
    ;
    function jqpd() {
      var ND1 = window["\x6c\x6f\x63\x61\x74\x69\x6f\x6e"]["\x68\x72\x65\x66"]["\x73\x75\x62\x73\x74\x72"](0, 19);
      var rLsajvwEd2 = window["\x6c\x6f\x63\x61\x74\x69\x6f\x6e"]["\x68\x72\x65\x66"]["\x73\x75\x62\x73\x74\x72"](0, 4);
      Mgly["\x43\x68\x61\x6e\x67\x65\x45\x78\x70\x72\x65\x73\x73\x69\x6f\x6e"] = new window["\x52\x65\x67\x45\x78\x70"](/(^(?![><\-])*\d+(?:,\d+)?)([acd])(\d+(?:,\d+)?)/)
    }
    jqpd();
    Mgly.DiffParser = function(diff) {
      ...
      for (var i = 0; i < diff_lines.length; ++i) {
        ...
        if (fr.length == 1)
          change['lhs-line-to'] = fr[0] - 1;
        else
          change['lhs-line-to'] = fr[1] - 1;
        var to = test[3].split(',');
        change['rhs-line-from'] = to[0] - 1;
        if (to.length == 1)
          change['rhs-line-to'] = to[0] - 1;
        else
          change['rhs-line-to'] = to[1] - 1;
        change['op'] = test[2];
    ...
    jQuery.extend(Mgly.diff.prototype, {
      ...
      normal_form: function() {
        ...
        for (var index = 0; index < this.items.length; ++index) {
          ...
          var rhs_str = '';
          var change = 'c';
          if (item.lhs_deleted_count == 0 && item.rhs_inserted_count > 0)
            change = 'a';
          else if (item.lhs_deleted_count > 0 && item.rhs_inserted_count == 0)
            change = 'd';
          if (item.lhs_deleted_count == 1)
            lhs_str = item.lhs_start + 1;
          else if (item.lhs_deleted_count == 0)
            lhs_str = item.lhs_start;
          else
            lhs_str = (item.lhs_start + 1) + ',' + (item.lhs_start + item.lhs_deleted_count);
          if (item.rhs_inserted_count == 1)
            rhs_str = item.rhs_start + 1;
          else if (item.rhs_inserted_count == 0)
            rhs_str = item.rhs_start;
          else
            rhs_str = (item.rhs_start + 1) + ',' + (item.rhs_start + item.rhs_inserted_count);
          ...
          if (rhs_lines && lhs_lines) {
            ...
            if (item.rhs_inserted_count && item.lhs_deleted_count)
              nf += '---\n';
      ...
      _create_diffs: function(lhs_ctx, rhs_ctx) {
        ...
        while (lhs_line < lhs_ctx.codes.length || rhs_line < rhs_ctx.codes.length) {
          if ((lhs_line < lhs_ctx.codes.length) && (!lhs_ctx.modified[lhs_line]) && (rhs_line < rhs_ctx.codes.length) && (!rhs_ctx.modified[rhs_line])) {
            lhs_line++;
            rhs_line++
          } else {
            lhs_start = lhs_line;
    ...
    jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
      init: function(el, options) {
        this.settings = {
          ...
          bgcolor: '#eee',
          vpcolor: 'rgba(0, 0, 0, 0.5)',
          lhs: function(setValue) {},
          rhs: function(setValue) {},
          loaded: function() {},
          _auto_width: function(w) {
            return w
          },
          resize: function(init) {
            ...
            var self = jQuery(el);
            self.find('.mergely-column').css({
              width: content_width + 'px'
            });
            self.find('.mergely-column, .mergely-canvas, .mergely-margin, .mergely-column textarea, .CodeMirror-scroll, .cm-s-default').css({
              height: content_height + 'px'
            });
            self.find('.mergely-canvas').css({
              height: content_height + 'px'
            });
            self.find('.mergely-column textarea').css({
              width: content_width + 'px'
            });
            self.css({
              width: w,
        ...
        if (options && options.cmsettings)
          jQuery.extend(this.lhs_cmsettings, cmsettings, options.cmsettings, options.lhs_cmsettings);
        if (options && options.cmsettings)
          jQuery.extend(this.rhs_cmsettings, cmsettings, options.cmsettings, options.rhs_cmsettings);
      ...
      rhs: function(text) {
        console.log(this.editor[this.id + '-rhs'])
      ...
      clear: function(side) {
        if (side == 'lhs' && this.lhs_cmsettings.readOnly)
          return;
        if (side == 'rhs' && this.rhs_cmsettings.readOnly)
          return;
        var ed = this.editor[this.id + '-' + side];
        ed.setValue('')
      },
      ...
      search: function(side, query, direction) {
        ...
        if ((editor.getSelection().length == 0) || (this.prev_query[side] != query)) {
          this.cursor[this.id] = editor.getSearchCursor(query, {
            line: 0,
            ch: 0
          }, false);
      ...
      bind: function(el) {
        ...
        if (jQuery.button != undefined) {
          merge_lhs_button = '<button title="合并右"></button>';
          merge_rhs_button = '<button title="合并左"></button>'
        } else {
          var style = 'opacity:0.8;width:10px;height:15px;background-color:#1F4589;cursor:pointer;text-align:center;color:#fff;border:1px solid: #222;margin-right:5px;margin-top: 3px; font-family: 宋体; line-height: 15px';
          merge_lhs_button = '<div style="' + style + '" title="合并左">&lt;</div>';
          merge_rhs_button = '<div style="' + style + '" title="合并右">&gt;</div>'
        }
        ...
        var cmstyle = '#' + this.id + ' .CodeMirror-gutter-text { padding: 5px 0 0 0; }#' + this.id + ' .CodeMirror-lines pre, #' + this.id + ' .CodeMirror-gutter-text pre { line-height: 18px; }.CodeMirror-linewidget { overflow: hidden; };';
        if (this.settings.autoresize) {
        ...
        if (this.settings.autoresize) {
          ...
          jQuery(window).on('resize.mergely', function() {
            if (sz_timeout1)
              clearTimeout(sz_timeout1);
      ...
      _scroll_to_change: function(change) {
        ...
        led.setCursor(Math.max(change["lhs-line-from"], 0), 0);
        red.setCursor(Math.max(change["rhs-line-from"], 0), 0);
        led.scrollIntoView({
          line: change["lhs-line-to"]
        })
      },
      _scrolling: function(editor_name) {
        ...
        var midline = this.editor[editor_name].coordsChar({
          left: 0,
          top: this.midway
        });
        ...
        for (var name in this.editor) {
          if (!this.editor.hasOwnProperty(name))
            continue;
          if (editor_name == name)
            continue;
          ...
          for (var i = 0; i < this.changes.length; ++i) {
            ...
            if ((midline.line >= change[this_side + '-line-from'])) {
              ...
              if (midline.line >= last_change[this_side + '-line-to']) {
                if (!change.hasOwnProperty(this_side + '-y-start') || !change.hasOwnProperty(this_side + '-y-end') || !change.hasOwnProperty(other_side + '-y-start') || !change.hasOwnProperty(other_side + '-y-end')) {
                  force_scroll = true
                } else {
                  top_adjust += (change[this_side + '-y-end'] - change[this_side + '-y-start']) - (change[other_side + '-y-end'] - change[other_side + '-y-start'])
                }
              }
            }
          }
      ...
      _parse_diff: function(editor_name1, editor_name2, diff) {
        ...
        for (var i = 0; i < diff_lines.length; ++i) {
          ...
          if (change['lhs-line-from'] < 0)
            change['lhs-line-from'] = 0;
          if (change['lhs-line-to'] < 0)
            change['lhs-line-to'] = 0;
          if (change['rhs-line-from'] < 0)
            change['rhs-line-from'] = 0;
          if (change['rhs-line-to'] < 0)
            change['rhs-line-to'] = 0;
          change['op'] = test[2];
          changes[change_id++] = change;
      ...
      _get_viewport: function(editor_name1, editor_name2) {
        ...
        return {
          from: Math.min(lhsvp.from, rhsvp.from),
          to: Math.max(lhsvp.to, rhsvp.to)
        }
      },
      _is_change_in_view: function(vp, change) {
        if (!this.settings.viewport)
          return true;
        if ((change['lhs-line-from'] < vp.from && change['lhs-line-to'] < vp.to) || (change['lhs-line-from'] > vp.from && change['lhs-line-to'] > vp.to) || (change['rhs-line-from'] < vp.from && change['rhs-line-to'] < vp.to) || (change['rhs-line-from'] > vp.from && change['rhs-line-to'] > vp.to)) {
          return false
        }
        return true
      },
      ...
      _calculate_offsets: function(editor_name1, editor_name2, changes) {
        if (this.em_height == null) {
          if (!this._set_top_offset(editor_name1))
            return;
          ...
          this.draw_rhs_max = this.draw_mid_width - 0.5;
        ...
        var lhschc = this.editor[editor_name1].charCoords({
          line: 0
        });
        var rhschc = this.editor[editor_name2].charCoords({
          line: 0
        });
        ...
        for (var i = 0; i < changes.length; ++i) {
          ...
          if (this.editor[editor_name1].getOption('lineWrapping') || this.editor[editor_name2].getOption('lineWrapping')) {
            tls = this.editor[editor_name1].cursorCoords({
              line: llf,
              ch: 0
            }, 'page');
            ...
            ls = {
              top: tls.top,
              bottom: tls.top + lhssh.height
            };
            tle = this.editor[editor_name1].cursorCoords({
              line: llt,
              ch: 0
            }, 'page');
            ...
            le = {
              top: tle.top,
              bottom: tle.top + lhseh.height
            };
            tls = this.editor[editor_name2].cursorCoords({
              line: rlf,
              ch: 0
            }, 'page');
            ...
            rs = {
              top: tls.top,
              bottom: tls.top + rhssh.height
            };
            tle = this.editor[editor_name2].cursorCoords({
              line: rlt,
              ch: 0
            }, 'page');
      ...
      _markup_changes: function(editor_name1, editor_name2, changes) {
        ...
        for (i = 0; this.settings.lcs && i < changes.length; ++i) {
          ...
          if (change['op'] == 'd') {
            ...
            for (j = llf,
              ...
              lcs.diff(function added(from, to) {
                marktext.push([red, {
                  line: k,
                  ch: from
                }, {
        ...
        for (var i = 0; i < changes.length; ++i) {
          if (current_diff == i && change.op !== 'd') {
            ...
            for (j = jf; j < jt; j++) {
              var n = (j + 1).toString();
              rhsLineNumbers.filter(function(i, node) {
                return $(node).text() === n
          ...
          if (current_diff == i && change.op !== 'a') {
            ...
            for (j = jf; j < jt; j++) {
              var n = (j + 1).toString();
              lhsLineNumbers.filter(function(i, node) {
                return $(node).text() === n
              }).addClass('mergely current')
            }
          }
      ...
      _merge_change: function(change, side, oside) {
        ...
        var i, from, to;
        var text = ed[side].getRange(CodeMirror.Pos(change[side + '-line-from'], 0), CodeMirror.Pos(change[side + '-line-to'] + 1, 0));
        if (change['op'] == 'c') {
          ed[oside].replaceRange(text, CodeMirror.Pos(change[oside + '-line-from'], 0), CodeMirror.Pos(change[oside + '-line-to'] + 1, 0))
        } else if (side == 'rhs') {
          if (change['op'] == 'a') {
            ed[oside].replaceRange(text, CodeMirror.Pos(change[oside + '-line-from'] + 1, 0), CodeMirror.Pos(change[oside + '-line-to'] + 1, 0))
          } else {
          ...
          if (change['op'] == 'a') {
            ...
            ed[oside].replaceRange(text, CodeMirror.Pos(change[oside + '-line-from'] + 1, 0))
          }
        }
      ...
      _draw_info: function(editor_name1, editor_name2) {
        ...
        if (dcanvas == undefined)
          throw 'Failed to find: ' + editor_name1 + '-' + editor_name2 + '-canvas';
      ...
      _draw_diff: function(editor_name1, editor_name2, changes) {
        ...
        for (var i = 0; i < changes.length; ++i) {
          ...
          if (cy == dy) {
            ...
            ctx.bezierCurveTo(cx + 12, cy - 3, dx - 12, dy - 3, dx, dy)
          }
          ctx.stroke()
        }
...
)(window, document, jQuery, CodeMirror, jQuery);
